...
//Insert story into the database
  $submitForm = mysql_query("INSERT INTO ". $tblpre ."fics (fanFicId, author, authorEmail, ficTitle, chapter, ficText, datePosted, valid)
  Values ('', '$userName2', '$userEmail2', '$userFic2', '$userChapter2', '$userText2','$date','False')",$connect);

// retrieve new fanFicId
//$result = mysql_query("SELECT fanFicId FROM ". $tblpre ."fics WHERE author = '$userName' AND authorEmail = '$userEmail' AND ficTitle = '$userFic' AND chapter = '$userChapter'",$connect);
$result = mysql_query("SELECT fanFicId FROM ". $tblpre ."fics WHERE ficText = '$userText2'",$connect);
while($myrow = mysql_fetch_array($result))
 {
  $newFicId = $myrow['fanFicId'];
 }
//Email the user that it has been submitted and will be reviewed
mail("$userName <$userEmail>",strip_gpc_slashes("Thank You For Submitting to $sitename"),strip_gpc_slashes("Dear $userName,\n\nThank you for submitting \"$userFic - $userChapter\" to KPFanFiction.com.\n\nCurrently it is inactive. Meaning it is not currently viewable by the public. When it has been verified by an editor, you will be notified accordingly.\n\nThank you.\n$site Editors"),"From: $site Editors <$editorEM>");
//Email the editors about the new fanfic to review.
mail("$site Editors <$editorEM>", strip_gpc_slashes("Fanfiction Story Submitted Needs Reviewing"), strip_gpc_slashes("Dear Editors,\n\nA submitted fanfiction story needs reviewing. You can read and verify/remove \"$userFic - $userChapter\" here:\n\nhttp://$site/editors/verify.php?fic=$newFicId\n\nThank you.\nKPFanFiction.com Team"), "From: $site <$webmasterEM>");
echo "<html><head><link rel=\"stylesheet\" href=\"includes/ffp.css\" type=\"text/css\"><title>Preview of $userFic - $userChapter</title></head>";
