...
if(isset ($_GET['p']))
 {
  if ($_GET['p'] == "submit")
   {
    $reviewForm = mysql_query("INSERT INTO ". $tblpre ."reviews (reviewId, fanFicId, review, starRating, userName, userEmail, datePosted)
    VALUES ('', '$fic', '$userReview', '$starLevel', '$userName', '$userEmail', now())",$connect);
    $result1 = mysql_query("SELECT * FROM ". $tblpre ."fics WHERE fanFicId = '$fic'",$connect);
    while($myrow1 = mysql_fetch_array($result1))
    {$authorEmail = $myrow1['authorEmail'];
    $author = $myrow1['author'];
    $ficTitle = $myrow1['ficTitle'];
    $chapter = $myrow1['chapter'];
    $headers = "From: $sitename <$editorEM>";
    mail("$author <$authorEmail>", strip_gpc_slashes("Review Submitted at $sitename"), strip_gpc_slashes("Dear $author,\n\nSomeone has submitted a review on your fanfiction \"$ficTitle - $chapter\".\n\nTo view the review(s) visit: http://www.$site/revfic.php?fic=$fic.\n\nThank you\n$sitename Team"), strip_gpc_slashes($headers));}
...
<?
$fic = $_REQUEST['fic'];
$result = mysql_query("SELECT COUNT(*) AS count FROM ". $tblpre ."reviews WHERE fanFicId = '$fic'",$connect);
