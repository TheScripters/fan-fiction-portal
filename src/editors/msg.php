...
if ($_GET['mode'] == "edit")
  {
    ...
    $post = $_REQUEST['p'];
    $result = mysql_query("SELECT * FROM ". $tblpre ."message WHERE MsgId = '$post'",$connect);
...
echo "<table>";
$result = mysql_query("SELECT * FROM ". $tblpre ."message ORDER BY MsgId DESC LIMIT 0,15",$connect);
while($row = mysql_fetch_array($result))
  {
    $MsgId = $row['MsgId'];
    $Name = $row['Name'];
