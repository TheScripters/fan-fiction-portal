...
if ($_GET['p'] == "update")
   {
     ...
     $RSSUpdate = mysql_query("UPDATE ". $tblpre ."config SET Value = '$rssvalue' WHERE Id = 'RSSLength'",$connect);
     header("Location: index.php?mode=rss");
   }
$rsscfg = mysql_query("SELECT Value FROM ". $tblpre ."config WHERE Id = 'RSSLength'",$connect);
...
echo "<h2>Preview RSS Feed</h2><center><table>";
    $result = mysql_query("SELECT * FROM ". $tblpre ."fics WHERE valid = 'True' ORDER BY fanFicId DESC LIMIT 0,$rss",$connect);
    while($myrow = mysql_fetch_array($result))
       {
         echo "<tr><td align=\"center\"><a href=\"http://$site/fics.php?fic=";
         echo $myrow['fanFicId'];
