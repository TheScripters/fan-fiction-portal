...
$result = mysql_query("SELECT * FROM ". $tblpre ."fics WHERE fanFicId = '$ficId'",$connect);
...
if ($valid == "true")
  {
    $verifyForm = mysql_query("UPDATE ". $tblpre ."fics SET valid = 'True', rating = '$rating' WHERE fanFicId = '$ficId'",$connect);
    //We need to tell the user their story was accepted.
    mail("$author <$authorEmail>","Story Accepted And Activated",strip_gpc_slashes("Dear $author,\n\nWe're happy to let you know your story \"$ficTitle - $ficChapter\" has been accepted and activated. You can view it here:\n\nhttp://$site/fics.php?fic=$ficId\n\nThank you.\n$sitename Team"),"From: $sitename Editors <$editorEM>");
  }
//If the validate form said "False" then delete the record altogether
if ($valid == "false")
  {
    $verifyForm = mysql_query("DELETE FROM ". $tblpre ."fics WHERE fanFicId = '$ficId' LIMIT 1",$connect);
    //Even though it was rejected, we need to tell the user so they aren't waiting on it.
    mail("$author <$authorEmail>","Story Rejected and Removed",strip_gpc_slashes("Dear $author,\n\nWe regret to inform you that your story was rejected from $sitename.\n\nIt was probably against the guidelines somehow. For more information, We would suggest contacting $editorEM to find out exactly why. Then you may try again by using the submission form.\n\nThank you.\n$sitename Team"),"From: $sitename Editors <$editorEM>");
  }
echo "<html><head><link rel=\"stylesheet\" type=\"text/css\" href=\"includes/ffp.css\"></head><body>";
