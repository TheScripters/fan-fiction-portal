...
include("../includes/db.php");
   ...
   $getpick = mysql_query("SELECT * FROM ". $tblpre ."config WHERE Id = 'EditorPick'",$connect);
while($editpick = mysql_fetch_array($getpick))
  {
    $editorspick = $editpick['Value'];
  }
$sql = mysql_query("SELECT * FROM ". $tblpre ."fics WHERE fanFicId = '$editorspick'",$connect);
...
<?
   ...
   $result3 = mysql_query("SELECT * FROM ". $tblpre ."fics WHERE valid = 'True' ORDER BY ficTitle",$connect);
