...
$checkForName = mysql_query("SELECT AuthorName FROM ". $tblpre ."authors WHERE AuthorName <> '$AuthorOldName'",$connect);
$nameFound = "False";
...
$result2 = mysql_query("UPDATE ". $tblpre ."authors SET AuthorName = '$AuthorNewName', AuthorEmail = '$AuthorNewEmail', AboutAuthor = '$AuthorAboutMe', Interests = '$AuthorInterest', Age = '$AuthorAge' WHERE AuthorName = '$AuthorOldName'",$connect);
if ($AuthorOldName != $AuthorNewName || $AuthorOldEmail != $AuthorNewEmail)
 $result3 = mysql_query("UPDATE ". $tblpre ."fics SET author = '$AuthorNewName', authorEmail = '$AuthorNewEmail' WHERE author = '$AuthorOldName' AND authorEmail = '$AuthorOldEmail'",$connect);
...
if ($OldPassword != "" && $NewPassword != "")
 {
  $result = mysql_query("SELECT password FROM ". $tblpre ."authors WHERE AuthorName = '$AuthorNewName'",$connect);
  ...
  if ($OldPassword == $AuthorsCorrectPassword || $md5OldPassword == $AuthorsCorrectPassword)
   {
    $result2 = mysql_query("UPDATE ". $tblpre ."authors SET Password = '$md5NewPassword' WHERE AuthorName = '$AuthorNewName'",$connect);
   }
  else
   {
